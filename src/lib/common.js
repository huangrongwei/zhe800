...
function startMove(obj, json, fnend) {
	...
	clearInterval(obj.timer);	//清楚obj定时器
...
var match={
	...
	psweasy: function(str) { //6-24位首字母开头
		var reg = /^[a-zA-Z0-9]\w{5,23}$/;
		return reg.test(str);
	},
...
var Cookie = {
	...
	remove:function(key,path){
        ...
        if(path){
            this.set(key,'no',{'expires':now,'path':path});//'no'：是因为该键将删除，所以写什么键值都行
        }else{
...
function ajax(method,path,data,fn){
